...
const PaymentDisplay: React.FC<PaymentDisplayProps> = ({ qr, invoice, status, error, onCopy, onBack }) => (
  <div className="flex flex-col items-center w-full">
    ...
    <div className="mt-4 text-center font-semibold w-full">
      {status.includes('✅') && (
        <Alert color="success" className="mb-2">{status}</Alert>
      )}
...
const PaymentSection: React.FC<PaymentSectionProps> = ({ lnInput, amount, onBack }) => {
  const [status, setStatus] = useState('Requesting invoice...');
  ...
  useEffect(() => {
    ...
    let tries = 0;
    async function poll() {
      while (pollingRef.current && tries < 60 && !paid) {
      ...
      if (!paid && pollingRef.current) setStatus('⏰ Payment not detected (timeout).');
    }
    poll();
